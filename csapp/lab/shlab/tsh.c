...
#define __USE_POSIX
 /**
  * wsl 环境下没有signal相关函数
  * 为什么在sdriver下跑这个程序，父子进程接收到的fork的返回值都不为0，但是跑出来又是对的
 */
...
int modifyjob(struct job_t *jobs, pid_t pid, int state);
...
void eval(char *cmdline)
{
    char *argv[MAXARGS]; /* parsed argv */
    char buf[MAXLINE];
    strcpy(buf, cmdline);
    pid_t bg = parseline(buf, argv);
    ...
    if (pid) /* father process */
    {
        ...
        else /* if fg, father process should wait for child process */
            waitfg(pid);
    }
    else  /* whatever bg of fg, child process should exec command */
    {
...
/* modifyjob - Modify state of a job whose PID=pid from the job list to state */
int modifyjob(struct job_t *jobs, pid_t pid, int state)
{
    int i;
